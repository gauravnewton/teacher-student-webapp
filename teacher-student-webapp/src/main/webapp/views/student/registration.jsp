	...
	<div class="row">
		...
		<div class="col-sm-8 registration-form">
			<h1 class="text-center">
				Student <strong>Registration</strong>
			</h1>
			<form class="form-horizontal" id="studentRegistrationForm">
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-12">
						<div>
							<small>Already have a account ? <a href="../student" class="register-link">Login Here.</a></small>
						</div>
					</div>
				</div>
				<div class="form-group">
...
<script>
	$(document).ready( function() {
		studentRegistrationFormValidate();
		
		var registerStudent = function(dataToPost){
			$.ajax({
				url: "../student/submitStudentRegistrationRequest",
				type: 'POST',
				data: dataToPost,
				crossDomain: true,
				dataType: 'json',
				...
				success: function(response) {
					if(response && response.statusCode == "OK"){
						...
						setTimeout(function(){ window.location.href = "../student"; }, 2000);
		...
		$("#register").on('click', function(){	
			...
			var dataToPost = serializeToJson($('#studentRegistrationForm').serialize());
			if( $('#studentRegistrationForm').valid() ){
				openLoader(options);
				registerStudent(dataToPost);	
			}
		});
